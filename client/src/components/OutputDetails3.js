...
//sonarqube
const OutputDetails3 = ({ outputDetails, data }) => {
  return (
    ...
    <div className="metrics-container mt-4 flex flex-col space-y-3">
      ...
      <p className="text-sm">
        ...
        <span className="font-semibold px-2 py-1 rounded-md bg-gray-100">
        {data?.sonarqube?.complexity?.cyclomatic }
        </span>
      </p>
      <p className="text-sm">
        Maintainability:{" "}
        <span className="font-semibold px-2 py-1 rounded-md bg-gray-100">
        {data?.sonarqube?.maintainability }
        </span>
      </p>
      <p className="text-sm">
       Difficulty:{" "}
        <span className="font-semibold px-2 py-1 rounded-md bg-gray-100">
        {data?.sonarqube?.complexity?.methodAggregate?.halstead?.difficulty }
        </span>
      </p>
      <p className="text-sm">
       Effort:{" "}
        <span className="font-semibold px-2 py-1 rounded-md bg-gray-100">
        {data?.sonarqube?.complexity?.methodAggregate?.halstead?.effort }
        </span>
      </p>
      <p className="text-sm">
       Length:{" "}
        <span className="font-semibold px-2 py-1 rounded-md bg-gray-100">
        {data?.sonarqube?.complexity?.methodAggregate?.halstead?.length }
        </span>
      </p>
      <p className="text-sm">
       Cyclomatic Density:{" "}
        <span className="font-semibold px-2 py-1 rounded-md bg-gray-100">
        {data?.sonarqube?.complexity?.methodAggregate?.cyclomaticDensity }
        </span>
      </p>
      <p className="text-sm">
        Halstead Bugs:{" "}
        <span className="font-semibold px-2 py-1 rounded-md bg-gray-100">
        {data?.sonarqube?.complexity?.methodAggregate?.halstead?.bugs }
        </span>
      </p>
      <p className="text-sm">
      Methods:
      <br/>
       {data?.sonarqube?.methods.map((method, index) => (
      ...
      <span className="font-semibold px-2 py-1 rounded-md ">
       {data?.sonarqube?.complexity?.methodAggregate?.cyclomatic <= 5 ? 
      <>
