...
const RetieveFeedbackForm = () => {
  const [code, setCodes] = useState([]);
  

  const fetchFeedbacks = async () => {
    try {
      const response = await axios.get('http://localhost:3001/code/getcode');
   ...
   return (
    ...
    <div className={`dropdown-content ${dropdownOpen ? 'show' : ''}`}>
     <a href={assignmentsUrl}>Assignments</a>
      ...
      <main>
        <div className="intro">
          <h1>Submitted Codes & Feedbacks</h1>
            </div>
...
export default RetieveFeedbackForm;
